...
module.exports = function(urls, options) {
	var zk = new ZkClient(urls, options);
	...
	cmds = {
		...
		exists: 'a_exists',
		get: 'a_get',
		getChildren: 'a_get_children',
		getChildren2: 'a_get_children2',
		set: 'a_set',
		delete: 'a_delete_'
	}
		...
		(function (cmd) {
			zk[cmd] = function() {
				...
				if(typeof(origcb) == 'function') {
					args[args.length - 1] = function(code, message){
						// console.log('xxxxxx');
						var error = null;
						if(code) {
						...
						var outargs = [error];
						outargs = outargs.concat(Array.prototype.slice.call(arguments, 2));

						origcb.apply(origcb, outargs);
					}


				}
				// console.log(arguments.length);
				client[cmds[cmd]].apply(client, args);
			};
		}(cmd));
